...
### Запуск процессов (Supervisor)
Внутри контейнера **dispatch_backend** все процессы управляются через **Supervisor** — он запускает, перезапускает и следит за сервисами:
| Процесс | Что делает |
| :--- | :--- |
| **django_init** | Применяет миграции и собирает статику (при старте) |
| **django_seed** | Создает администратора, системного бота и базовые настройки |
| **gunicorn** | WSGI/ASGI-сервер Django (4 воркера, порт 8000) |
| **cron** | Планировщик периодических задач |
| **parsing_inbox_emails** | Парсинг входящей почты; автоматическая регистрация инцидентов |
| **parsing_sent_emails** | Синхронизация исходящих писем |
> **Gunicorn** + **Uvicorn** — гибридный подход: Gunicorn управляет процессами, Uvicorn обеспечивает асинхронную обработку (WebSocket). 4 воркера = 4 параллельных запроса.
