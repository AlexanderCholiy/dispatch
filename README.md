...
**Логи** (`./logs/`):
- Django, Celery, Grafana, Nginx, Supervisor
- Автоматическая ротация — защита от переполнения диска
**Данные** (`./data/`):
- Временные файлы отчетов
- Экспортированные данные
- Кэш тяжелых операций

> [!NOTE]
> Папки **`./logs/`** и **`./data/`** монтируются в контейнеры, поэтому все логи и данные доступны прямо в проекте — не нужно заходить в Docker.
