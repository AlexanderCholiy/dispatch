...
Для идентификации переписок используется код инцидента в теме письма.

Даже при изменении темы сохраняется связь с исходным инцидентом.

При отправке писем из Yandex Tracker они уходят как новые сообщения, поэтому код инцидента обязателен.
