...
export function startStatisticsWebSocket(charts) {
    ...
    const durationInput = document.getElementById('incident-duration-min'); // Новый инпут
    ...
    durationInput.value = ''; // Пусто по умолчанию
    endInput.max = new Date().toISOString().split('T')[0];
    
    let confirmedStart = defaultStart;
    let confirmedEnd = null;
    let confirmedDuration = null; // Храним значение длительности или null
    ...
    function sendParams() {
        ...
        // Добавляем длительность только если она установлена и > 0
        if (confirmedDuration !== null && confirmedDuration > 0) {
            payload.incident_duration_min = confirmedDuration;
        }

        if (confirmedResponsible !== '') {
            payload.responsible_user = confirmedResponsible === 'none' ? null : confirmedResponsible;
        }
    ...
    function updateCharts(apiData, avrData = null, dispatchData = null) {
        // ... (остальной код updateCharts без изменений, так как он зависит от API, а не от UI напрямую)
        ...
        const needRebuildWeekly = startDate !== lastWeeklyStart || endDate !== lastWeeklyEnd;
        ...
        if (avrTableContainer && avrData) {
            const needRebuildAvr = startDate !== lastAvrStart || endDate !== lastAvrEnd;
            if (avrData.error) {
        ...
        updateBarChart(charts.closed, apiData, [
            'total_closed_incidents', 'closed_incidents_with_power_issue'
        ], macroregionLabels);
        ...
        updateBarChart(charts.open, apiData, [
            'total_open_incidents', 'open_incidents_with_power_issue'
        ], macroregionLabels);
        ...
        updateBarChart(charts.types, apiData, [
            'is_power_issue_type', 'is_ams_issue_type', 'is_goverment_request_issue_type',
            'is_vols_issue_type', 'is_object_destruction_issue_type', 'is_object_access_issue_type'
        ], macroregionLabels);
        updateCategoryTotals(charts.types, [
            { id: 'total-power', index: 0 }, { id: 'total-ams', index: 1 },
            { id: 'total-government', index: 2 }, { id: 'total-vols', index: 3 },
            { id: 'total-destruction', index: 4 }, { id: 'total-access', index: 5 },
        ]);
        ...
        if (charts.subtypes?.power) {
            updateSubtypesChart(
                charts.subtypes.power.chart, apiData, 'Аварии по питанию',
                charts.subtypes.power.labels, macroregionLabels
            );
            updateCategoryTotals(charts.subtypes.power.chart, [
                { id: 'eo-nb-vl-1kv', index: 0 }, { id: 'eo-nb-vl-1kv-plus', index: 1 },
                { id: 'eo-nb-kl-1kv', index: 2 }, { id: 'eo-nb-kl-1kv-plus', index: 3 },
                { id: 'eo-nb-ktp', index: 4 }, { id: 'eo-nb-pu', index: 5 },
                { id: 'eo-nb-rsh', index: 6 }, { id: 'eo-operator-line', index: 7 },
                { id: 'eo-operator-eu', index: 8 }, { id: 'eo-so-emergency', index: 9 },
                { id: 'eo-so-planned', index: 10 }, { id: 'eo-other', index: 11 },
                { id: 'eo-no-payment', index: 12 }, { id: 'eo-self-recovery', index: 13 },
                { id: 'eo-shep', index: 14 }, { id: 'eo-no-scheme', index: 15 },
            ...
            updateCopyButton(
                'energy-subtypes-chart-card', charts.subtypes.power.chart, 'Регион/Подкатегория аварии по питанию'
            );
        }
    ...
    applyBtn.addEventListener('click', () => {
        ...
        if (durationRaw !== '') {
            const numVal = Number(durationRaw);
            if (!Number.isInteger(numVal) || numVal < 1) {
                return showMessage('TTL инцидента должен быть целым числом >= 1 минуты', 'warning', messagesContainer, lastMsgRef);
            }
            validDuration = numVal;
        }

        // Сохраняем подтвержденные значения
        confirmedStart = start;
        confirmedEnd = end;
        confirmedDuration = validDuration; // Может быть null или число
    ...
    resetBtn.addEventListener('click', () => {
        startInput.value = defaultStart;
        endInput.value = '';
        durationInput.value = ''; // Очищаем поле длительности
        responsibleSelect.value = '';
        operatorSelect.value = '';

        confirmedStart = defaultStart;
        confirmedEnd = null;
        confirmedDuration = null; // Сбрасываем длительность
